...
void main() {
// vec3 color = render();
// fragColor = vec4(0.001 * gl_FragCoord.x, 0.001 * gl_FragCoord.y, 0.3, 1.0); //vec4(1.0 * gl_FragCoord);
fragColor = vec4(0.001, 0.001, 0.3, 1.0); //vec4(1.0 * gl_FragCoord);
}
...
// #define RAD 1.0 * 0.01745329251
// #define EPSILON 1.0 * 0.001
...
//   if (t < 1.0)return.5 * (t * t*(((s *= (1.525)) + 1.0) * t-s));
//   return.5 * ((t -= 2.0) * t*(((s *= (1.525)) + 1.0) * t+s) + 2.0);
// }
// // Bounce
...
//   t = t*2.0; if (t < 1.0)return.5 * t*t * t;
//   return.5 * ((t -= 2.0) * t*t + 2.0);
// }
// // Elastic
...
//   if (t == 0.0)return 0.0;
//   if (t == 1.0)return 1.0;
//   if (t < 1.0)return.5 * pow(2.0, 10.0 * (t - 1.0));
//   return.5 * (-pow(2.0, - 10.0 *-- t) + 2.0);
// }
// // Quadratic
...
//   t = t*2.0; if (t < 1.0)return.5 * t*t * t*t;
...
//   t = t*2.0; if (t < 1.0)return.5 * t*t * t*t * t;
//   return.5 * ((t -= 2.0) * t*t * t*t + 2.0);
// }
// // Sine
...
// #define CAMERA_FOV 1.0 * 0.90
// #define CAMERA_NEAR 1.0 * 0.000001
